...
namespace TinyGiantStudio.ModularToDoLists
{
    ...
    {
        readonly string versionNumber = "1.3.1";
        ...
        int indentForTaskSubOptions = 50;
        ...
        [MenuItem("Tools/Tiny Giant Studio/Modular To-do Lists")]
            ...
            GUILayout.BeginVertical();
            {
                GUILayout.BeginHorizontal();
                {
                    ...
                    GUILayout.BeginVertical(); //main content goes here
                    {
                        ...
                        GUILayout.BeginHorizontal();
                        {
                            EditorGUILayout.LabelField("Page Transition Speed", myStyleToggle, GUILayout.Width(220));
                        ...
                        GUILayout.BeginHorizontal();
                        {
                            EditorGUILayout.LabelField("Task is considered due soon if less than", myStyleToggle, GUILayout.Width(250));
                        ...
                        GUILayout.BeginHorizontal();
                        {
                            EditorGUILayout.LabelField(new GUIContent("Default task due date when created is, current time + "), myStyleToggle, GUILayout.Width(250));
                            ...
                            GUILayout.Space(5);
                            EditorGUILayout.LabelField("Hours", myStyleToggle, GUILayout.Width(220));
        ...
        void ListOfTasks(int listIndex, float originalAlpha)
        {
            ...
            ToDoList selectedToDoList = selectedTopic.toDoLists[listIndex];
        ...
        {
            if (task.completed)
                GUI.color = new Color(0.9f, 1f, 0.9f, originalAlpha);
            else if (task.failed)
                GUI.color = new Color(1f, 0.8f, 0.8f, originalAlpha);
            else if (task.ignored)
                GUI.color = new Color(1f, 1, 1, originalAlpha * 0.7f);
            else
                GUI.color = new Color(1f, 1f, 1f, originalAlpha);
            ...
            if (EditorGUIUtility.currentViewWidth > 450)
            {
                ...
                GUILayout.BeginHorizontal(myStyleTaskNumbering, GUILayout.Width(maxWidth));
                GUILayout.Label(taskIndex.ToString(), myStyleTaskLabel, GUILayout.Width(maxWidth));
                GUILayout.EndHorizontal();
            }
        ...
        private void DrawTaskDueDateEdit(Task task, bool flexibleIndent = false)
        {
        ...
        void TaskDescription(Task task, int indent = 50)
        {
            GUILayout.Space(4);
            if (task.editing)
            {
        ...
        bool TaskButtons(ToDoList todoList, Task task, bool forceTextLabels = false, bool hideIgnoreButton = false, bool hideOptions = false, bool showDoneButton = true)
        {
            ...
            removeButtonContent.tooltip = "Remove this card permanently.";
        ...
        void TaskName(ToDoList toDoList, Task task, bool showEditButton = true)
        {
            if (task.editing)
            {
                if (showEditButton)
                {
                    if (GUILayout.Button(editIcon, myStyleIcon, GUILayout.Height(15), GUILayout.Width(15)))
                    {
                        Undo.RecordObject(selectedTopic, selectedTopic.name + " update");
                        selectedTopic.Clicked();
                        task.editing = false;
        ...
        void BoardProgressBar(Topic topic)
        {
            ...
            if (incompleteTaskCount != 0)
                ProgressBar(incompleteTaskCount, totalTaskCount, "Remaining: ");

            GUILayout.Space(7);

            int completedTasksCount = topic.GetCompletedTaskCount();
            if (completedTasksCount != 0)
                ProgressBar(completedTasksCount, totalTaskCount, "Completed: ");

            GUILayout.Space(7);

            int failedTasksCount = topic.GetFailedTaskCount();
            if (failedTasksCount != 0)
                ProgressBar(failedTasksCount, totalTaskCount, "Failed: ");

            GUILayout.Space(indentRight + 2);
            GUILayout.EndHorizontal();
        }

        void ProgressBar(int value, int total, string progressbarType)
        {
            string fulllabel;
            ...
            EditorGUI.DrawPreviewTexture(new Rect(r.x, r.y, r.width * percentage * guiColor.a * guiColor.a, r.height), progressbarTexture);
            //EditorGUI.LabelField(new Rect(r.x, r.y, r.width, r.height), progressbarType + value, myStyleProgressbarLabel);
            EditorGUI.LabelField(new Rect(r.x, r.y, r.width, r.height), fulllabel, myStyleProgressbarLabel);
        ...
        void GenerateStyle()
        {
            ...
            if (myStyleTaskTwo == null)
            {
                ...
                myStyleTaskTwo.normal.background = Texture2D.grayTexture;
            }
            ...
            //if (myStyleTaskNumbering == null)
            {
                ...
                myStyleTaskNumbering.normal.background = backgroundTexture;
                myStyleTaskNumbering.padding = new RectOffset(0, 0, 0, 0);
