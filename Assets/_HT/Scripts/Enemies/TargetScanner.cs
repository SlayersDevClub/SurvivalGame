...
using UnityEngine;
    ...
    public PlayerStateMachine Detect(Transform detector, PlayerStateMachine detected, bool useHeightDifference = true) {

        // Check if detected.respawning is true
        if (detected.respawning) {
            return null;
        }

        Vector3 eyePos = detector.position + Vector3.up * heightOffset;
        Vector3 toPlayer = detected.transform.position - eyePos;
        Vector3 toPlayerTop = detected.transform.position + Vector3.up * 1.5f - eyePos;

        // Check if useHeightDifference condition is met
        ...
        // Debug when none of the conditions are met
